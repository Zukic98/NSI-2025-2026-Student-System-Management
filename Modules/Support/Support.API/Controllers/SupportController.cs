...
namespace Support.API.Controllers
{
    ...
    public class SupportController : ControllerBase
    {
        ...
        {
            var studentExists = await _facultyDbContext.Students.AnyAsync(s =>
                s.UserId == dto.UserId
            );
            ...
            var facultyExists = await _universityDbContext.Faculties.AnyAsync(f =>
                f.Id == dto.FacultyId
            );
        ...
        )
        {
            ...
            if (duplicate)
                return BadRequest(
                    "You already have an enrollment request for the same academic year and semester."
                );
            ...
            return CreatedAtAction(
                nameof(GetEnrollmentRequestById),
                new { id = request.Id },
                request
            );
        }
        ...
        {
            var req = await _dbContext
                .EnrollmentRequests.AsNoTracking()
        ...
        public async Task<IActionResult> GetPendingEnrollmentRequestsForFaculty(
        ...
        public async Task<IActionResult> ApproveEnrollmentRequest(
            [FromRoute] Guid id,
            [FromBody] DecideEnrollmentRequestDTO? dto
        )
        {
            var req = await _dbContext.EnrollmentRequests.FirstOrDefaultAsync(r => r.Id == id);
            ...
            if (req.Status != "Pending")
                return BadRequest(
                    $"Only Pending requests can be approved. Current status: {req.Status}"
                );
        ...
        public async Task<IActionResult> RejectEnrollmentRequest(
            [FromRoute] Guid id,
            [FromBody] DecideEnrollmentRequestDTO? dto
        )
        {
            ...
            if (req.Status != "Pending")
                return BadRequest(
                    $"Only Pending requests can be rejected. Current status: {req.Status}"
                );
