...
public class StudentExamRegistrationService : IStudentExamRegistrationService
{
    ...
    public async Task<ExamRegistrationResponseDto> RegisterAsync(
        int examId,
        string userId,
    ...
    )
    {
        if (examId <= 0)
        {
            throw new FacultyApplicationException(
                "A valid exam identifier must be provided.",
        ...
        var exam =
            await _repository.GetExamWithDetailsAsync(examId, cancellationToken)
            ?? throw new FacultyApplicationException(
                "Requested exam does not exist.",
        ...
        {
            throw new FacultyApplicationException(
                "Cannot register for an exam that has already concluded.",
                HttpStatusCode.BadRequest
            );
        }
        ...
        {
            throw new FacultyApplicationException(
                "Registration window for this exam is closed.",
                HttpStatusCode.BadRequest
            );
        }
        ...
        var alreadyRegistered = await _repository.HasExistingRegistrationAsync(
            student.Id,
            exam.Id,
            cancellationToken
        );
    ...
    public async Task<IReadOnlyList<AvailableExamDto>> GetAvailableExamsAsync(
        string userId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var enrolledCourseIds = student.Enrollments.Select(e => e.CourseId).Distinct().ToArray();

        var available = await _repository.GetEligibleExamsAsync(
            student.Id,
            enrolledCourseIds,
            DateTime.UtcNow,
            cancellationToken
        ...
        return available
            .Select(exam => new AvailableExamDto
            {
                ExamId = exam.Id,
    ...
    public async Task<IReadOnlyList<RegisteredExamDto>> GetRegistrationsAsync(
        string userId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        return registrations
            .Select(registration => new RegisteredExamDto
            {
