...
{
    Task<Student?> GetStudentByUserIdAsync(
        string userId,
        CancellationToken cancellationToken = default
    );
    Task<Exam?> GetExamWithDetailsAsync(int examId, CancellationToken cancellationToken = default);
    Task<bool> HasExistingRegistrationAsync(
        int studentId,
        int examId,
        CancellationToken cancellationToken = default
    );
    Task<ExamRegistration> SaveRegistrationAsync(
    ...
    Task<IReadOnlyList<ExamRegistration>> GetRegistrationsAsync(
        int studentId,
        CancellationToken cancellationToken = default
    );
}
