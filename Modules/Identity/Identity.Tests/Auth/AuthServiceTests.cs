...
namespace Identity.Tests.Auth
{
    public class AuthServiceTests
    {
        ...
        public async Task AuthenticateAsync_ShouldReturnTokens_WhenCredentialsAreValid()
        {
            ...
            _mockHasher
                .Setup(x => x.VerifyPassword(user, password, It.IsAny<string>()))
                .Returns(true);

            _mockTokenService
                .Setup(x => x.GenerateAccessToken(It.IsAny<TokenClaims>()))
                .Returns(accessToken);

            _mockTokenService.Setup(x => x.CreateRefreshToken(user.Id)).Returns(refreshToken);
        ...
        public async Task AuthenticateAsync_ShouldThrowUnauthorized_WhenPasswordIncorrect()
        {
            ...
            _mockHasher
                .Setup(x => x.VerifyPassword(user, "wrongpass", It.IsAny<string>()))
                .Returns(false);

            // Act & Assert
            var act = async () =>
                await _authService.AuthenticateAsync("test@test.com", "wrongpass");
