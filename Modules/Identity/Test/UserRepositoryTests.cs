...
public class UserRepositoryTests : IDisposable
{
    ...
    {
        _context.DomainUsers.Add(
            User.Create("teacher_a", string.Empty, "Teacher", "A", FacultyA, UserRole.Teacher)
                .SetId(Guid.NewGuid())
        );

        _context.DomainUsers.Add(
            User.Create("student_b", string.Empty, "Student", "B", FacultyB, UserRole.Student)
                .SetId(Guid.NewGuid())
        );

        _context.DomainUsers.Add(
            User.Create("student_a", string.Empty, "Student", "A", FacultyA, UserRole.Student)
                .SetId(Guid.NewGuid())
        );
