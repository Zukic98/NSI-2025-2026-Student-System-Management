...
public class UserServiceTests
{
    ...
    public async Task UpdateUserAsync_ShouldReturnTrueAndPublishEvent_WhenRoleChanges()
    {
        ...
        _mockPublisher.Verify(
            p =>
                p.PublishAsync(
                    It.Is<UserRoleAssignedEvent>(e =>
                        e.PreviousRole == oldRole && e.NewRole == newRole
                    )
                ),
            Times.Once
        );
    ...
    public async Task DeactivateUserAsync_ShouldReturnTrue_WhenUserIsActive()
    {
        ...
        Assert.True(result);
        _mockRepo.Verify(
            r => r.UpdateAsync(It.Is<User>(u => u.Status == UserStatus.Inactive)),
            Times.Once
        );
        _mockRepo.Verify(r => r.SaveAsync(), Times.Once);
    }
    ...
    public async Task DeleteUserAsync_ShouldPublishEvent_OnSuccess()
    {
        ...
        _mockPublisher.Verify(
            p => p.PublishAsync(It.Is<UserDeletedEvent>(e => e.UserId == userId)),
            Times.Once
        );
    }
    ...
    public async Task GetAllUsersAsync_ShouldPassFilterToRepoAndReturnList()
    {
        ...
        {
            User.Create("u1", string.Empty, "A", "B", facultyId, UserRole.Student, "001")
                .SetId(Guid.NewGuid()),
            User.Create("u2", string.Empty, "C", "D", facultyId, UserRole.Student, "002")
                .SetId(Guid.NewGuid()),
            User.Create("u3", string.Empty, "E", "F", facultyId, UserRole.Student, "003")
                .SetId(Guid.NewGuid()),
        };
        var totalCount = 50;

        _mockRepo
            .Setup(r => r.GetAllFilteredAsync(It.IsAny<UserFilterRequest>()))
            .ReturnsAsync(mockUsers.AsReadOnly());

        _mockRepo.Setup(r => r.CountAsync(It.IsAny<UserFilterRequest>())).ReturnsAsync(totalCount);
        ...
        _mockRepo.Verify(
            r =>
                r.GetAllFilteredAsync(
                    It.Is<UserFilterRequest>(f =>
                        f.FacultyId == facultyId && f.SortBy == "username"
                    )
                ),
            Times.Once
        );
